...
export default function ParticipantThumbnail({ userName, isMuted, videoRef }: ParticipantThumbnailProps) {
  ...
  return (
    <div 
      className={`fixed z-10 w-48 rounded-xl overflow-hidden shadow-2xl border border-white/20 bg-black/20 backdrop-blur-md aspect-video ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
      style={{
        left: `${position.x}px`,
        top: `${position.y}px`,
